		...
		try 
		{
			...
			{ // talker is a server; Mobile device will be Announcing UDP + Will be Server
				if (ip.empty())
				{
					...
					PVR_DB_I("[TCPTalker::TCPTalker] Talker is Server and ip is Empty. Accepting All connection...");
				}
				else
				{
					acc.async_accept(skt, errHdl); // todo: simplify
			...
			if (ec.value() == 0) 
			{
				...
				function<void(const asio::error_code &, size_t)> handle = [&](const asio::error_code &/*err*/, size_t len) 
				{
					...
					while (loop) 
					{
						auto it = find_first_of(stream.begin(), stream.end(), prefix.begin(), prefix.end());
						if (stream.end() - it >= 6) 
						{
							msgLen = size_t(*(it + 4)) + size_t(*(it + 5)) * 0x100;
							it += 6; // skip prefix
							if (stream.size() >= it - stream.begin() + msgLen) 
							{ // warning! do not increment iterator out of bounds
