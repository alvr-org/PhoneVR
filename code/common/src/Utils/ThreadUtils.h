...
inline void
EndThread(std::thread *&thr)   // get reference to pointer to be able to assign nullptr to thr
{
    if (thr) {
        thr->join();
        delete thr;
...
class TimeBomb {
  ...
  public:
    ...
    TimeBomb(
        std::chrono::microseconds timer, std::function<void()> boomCb = [] {})
        : cb(boomCb), tm(timer), loop(true) {}
...
class RefWhistle {
  ...
  public:
    ...
    void unblockNow() { perturbation(-lt); }
};