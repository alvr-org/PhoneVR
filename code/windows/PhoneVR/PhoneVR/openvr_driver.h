...
#pragma pack(push, 8)
    ...
    enum ETextureType {
        ...
        TextureType_OpenGL = 1,      // Handle is an OpenGL texture name or an OpenGL render buffer
                                     // name, depending on submit flags
    ...
    enum EColorSpace {
        ColorSpace_Auto =
            0,   // Assumes 'gamma' for 8-bit per component formats, otherwise 'linear'.  This
                 // mirrors the DXGI formats which have _SRGB variants.
        ColorSpace_Gamma = 1,   // Texture data can be displayed directly on the display without any
                                // conversion (a.k.a. display native format).
        ColorSpace_Linear = 2,   // Same as gamma but has been converted to a linear representation
                                 // using DXGI's sRGB conversion algorithm.
    };
...
#define INVALID_SHARED_TEXTURE_HANDLE ((vr::SharedTextureHandle_t) 0)
    ...
    enum ETrackedDeviceClass {
        ...
        TrackedDeviceClass_DisplayRedirect =
            5,   // Accessories that aren't necessarily tracked themselves, but may redirect video
    ...
    enum ETrackingUniverseOrigin {
        ...
        TrackingUniverseStanding =
            1,   // Poses are provided relative to the safe bounds configured by the user
        TrackingUniverseRawAndUncalibrated =
            2,   // Poses are provided in the coordinate system defined by the driver.  It has Y up
                 // and is unified for devices of the same driver. You usually don't want this one.
    };
    ...
    enum ETrackedDeviceProperty {
        ...
        Prop_IconPathName_String = 5000,   // DEPRECATED. Value not referenced. Now expected to be
                                           // part of icon path properties.
        Prop_NamedIconPathDeviceOff_String =
            5001,   // {driver}/icons/icon_filename - PNG for static icon, or GIF for animation,
                    // 50x32 for headsets and 32x32 for others
     ...
     * Submit. */
    enum EVRSubmitFlags {
        // Simple render path. App submits rendered left and right eye images with no lens
        ...
        // applied). The app would have used the data returned by vr::IVRSystem::ComputeDistortion()
        // to apply the correct distortion to the rendered images before calling Submit().
        Submit_LensDistortionAlreadyApplied = 0x01,

        // If the texture pointer passed in is actually a renderbuffer (e.g. for MSAA in OpenGL)
        // then set this flag.
    ...
    enum EVREventType {
        ...
        VREvent_RenderToast =
            507,   // Send to the dashboard to render a toast - data is the notification ID
        VREvent_ImageLoaded = 508,    // Sent to overlays when a SetOverlayRaw or SetOverlayFromFile
                                      // call finishes loading
    ...
    // fires when the devices transitions from Standby -> UserInteraction or Idle ->
    // UserInteraction. VREvent_TrackedDeviceUserInteractionEnded fires when the devices transitions
    ...
    struct VREvent_Scroll_t {
        float xdelta, ydelta;   // movement in fraction of the pad traversed since last delta, 1.0
                                // for a full swipe
        uint32_t repeatCount;
    };

    /** when in mouse input mode you can receive data from the touchpad, these events are only sent
    if the users finger is on the touchpad (or just released from it)
    **/
...
#pragma pack(pop)
    ...
    struct VRControllerAxis_t {
        float x;   // Ranges from -1.0 to 1.0 for joysticks and track pads. Ranges from 0.0 to 1.0
                   // for triggers were 0 is fully released.
        float y;   // Ranges from -1.0 to 1.0 for joysticks and track pads. Is always 0.0 for
                   // triggers.
    };
...
#pragma pack(push, 4)
    ...
    struct VRControllerState001_t {
        // If packet num matches that on your prior call, then the controller state hasn't been
        // changed since your last call and there is no need to process it
...
#pragma pack(pop)
    ...
    enum EVRInitError {
        ...
        // VRInitError_Driver_HmdDisplayNotFoundAfterFix	 = 210, // not needed: here for
        // historic reasons
    ...
    enum EVRTrackedCameraFrameType {
        ...
        VRTrackedCameraFrameType_Undistorted,   // In pixels, an undistorted inscribed rectangle
                                                // region without invalid regions. This size is
                                                // subject to changes shortly.
        VRTrackedCameraFrameType_MaximumUndistorted,   // In pixels, maximum undistorted with
                                                       // invalid regions. Non zero alpha component
                                                       // identifies valid regions.
...
#define MAX_CAMERA_FRAME_SHARED_HANDLES 4
    ...
    VR_CAMERA_DECL_ALIGN(8) struct CameraVideoStreamFrame_t {
        ...
        double m_flFrameCaptureTime_ClientRelative;     // At the client, relative to when the frame
                                                        // was exposed/captured.

        double m_flSyncMarkerError;

        TrackedDevicePose_t
            m_StandingTrackedDevicePose;   // Supplied by HMD layer when used as a tracked camera
...
namespace vr {
    enum EVRSettingsError {
        ...
        VRSettingsError_UnsetSettingHasNoDefault =
            5,   // This will be returned if the setting does not appear in the appropriate default
                 // file and has not been set
    };
    ...
    class IVRSettings {
      public:
        ...
        // resources/settings/ directory of either the runtime or the driver_xxx directory.
        // Otherwise the default will be false, 0, 0.0 or ""
...
namespace vr {
    ...
    class ITrackedDeviceServerDriver {
      public:
        ...
        /** Handles a request from the system to put this device into standby mode. What that means
         * is defined per-device. */
        virtual void EnterStandby() = 0;

        /** Requests a component interface of the driver for device-specific functionality. The
         * driver should return NULL if the requested interface or version is not supported. */
        virtual void *GetComponent(const char *pchComponentNameAndVersion) = 0;

        /** A VR Client has made this debug request of the driver. The set of valid requests is
         * entirely up to the driver and the client to figure out, as is the format of the response.
         * Responses that exceed the length of the supplied buffer should be truncated and null
         * terminated */
...
namespace vr {
    ...
    class IVRDisplayComponent {
      public:
        ...
        virtual void GetProjectionRaw(
            EVREye eEye, float *pfLeft, float *pfRight, float *pfTop, float *pfBottom) = 0;

        /** Returns the result of the distortion function for the specified eye and input UVs. UVs
         * go from 0,0 in the upper left of that eye's viewport and 1,1 in the lower right of that
         * eye's viewport. */
...
namespace vr {
    ...
    //			without allowing the VR Compositor to own the window/device. Chances are you
    // don't 			need to implement this component in your driver.
    // ----------------------------------------------------------------------------------------------
    class IVRDriverDirectModeComponent {
      public:
        ...
        /** Specific to Oculus compositor support, textures supplied must be created using this
         * method. */
        ...
        /** Used to textures created using CreateSwapTextureSet.  Only one of the set's handles
         * needs to be used to destroy the entire set. */
         ...
         * must be created using CreateSwapTextureSet and should be alternated per frame.  Call
...
namespace vr {
    ...
    class IVRCameraComponent {
      public:
        ...
        virtual bool
        SetCameraVideoStreamFormat(vr::ECameraVideoStreamFormat nVideoStreamFormat) = 0;
        ...
        virtual bool GetCameraDistortion(float flInputU,
                                         float flInputV,
                                         float *pflOutputU,
                                         float *pflOutputV) = 0;
...
namespace vr {
    ...
    class IVRDriverContext {
      public:
        /** Returns the requested interface. If the interface was not available it will return NULL
         * and fill out the error. */
    ...
    class IServerTrackedDeviceProvider {
      public:
         ...
         *
         * pDriverHost will never be NULL, and will always be a pointer to a IServerDriverHost
         *interface
         *
...
namespace vr {
    ...
    class IVRProperties {
      public:
        ...
        /** returns a string that corresponds with the specified property error. The string will be
         * the name of the error enum value for all valid error codes */
    ...
    class CVRPropertyHelpers {
      public:
        ...
        /** Returns a scaler property. If the device index is not valid or the property value type
         * does not match, this function will return false. */
        ...
        /** Returns a single typed property. If the device index is not valid or the property is not
         * a string type this function will return 0. Otherwise it returns the length of the number
         * of bytes necessary to hold this string including the trailing null. Strings will always
         * fit in buffers of k_unMaxPropertyStringSize characters. */
        ...
        /** Returns a string property. If the device index is not valid or the property is not a
         * string type this function will return 0. Otherwise it returns the length of the number of
         * bytes necessary to hold this string including the trailing null. Strings will always fit
         * in buffers of k_unMaxPropertyStringSize characters. */
        ...
        /** Returns a string property as a std::string. If the device index is not valid or the
         * property is not a string type this function will return an empty string. */
        std::string GetStringProperty(vr::PropertyContainerHandle_t ulContainer,
                                      vr::ETrackedDeviceProperty prop,
                                      vr::ETrackedPropertyError *peError = nullptr);

        /** Sets a scaler property. The new value will be returned on any subsequent call to get
        ...
        /** Sets a string property. The new value will be returned on any subsequent call to get
         * this property in any process. */
        ETrackedPropertyError SetStringProperty(PropertyContainerHandle_t ulContainerHandle,
                                                ETrackedDeviceProperty prop,
                                                const char *pchNewValue);

        /** Sets a single typed property. The new value will be returned on any subsequent call to
         * get this property in any process. */
        ...
        /** Sets the error return value for a property. This value will be returned on all
         * subsequent requests to get the property */
     ...
     * necessary to hold this string including the trailing null. Strings will always fit in buffers
     * of k_unMaxPropertyStringSize characters. */
    inline uint32_t
    CVRPropertyHelpers::GetStringProperty(PropertyContainerHandle_t ulContainerHandle,
                                          ETrackedDeviceProperty prop,
                                          VR_OUT_STRING() char *pchValue,
                                          uint32_t unBufferSize,
                                          ETrackedPropertyError *pError) {
    ...
    inline std::string
    CVRPropertyHelpers::GetStringProperty(vr::PropertyContainerHandle_t ulContainer,
                                          vr::ETrackedDeviceProperty prop,
    ...
    inline uint64_t
    CVRPropertyHelpers::GetUint64Property(PropertyContainerHandle_t ulContainerHandle,
                                          ETrackedDeviceProperty prop,
                                          ETrackedPropertyError *pError) {
        return GetPropertyHelper<uint64_t>(
            ulContainerHandle, prop, pError, 0, k_unUint64PropertyTag);
    }
    ...
    inline ETrackedPropertyError CVRPropertyHelpers::SetBoolProperty(
        ...
        return SetProperty(
            ulContainerHandle, prop, &bNewValue, sizeof(bNewValue), k_unBoolPropertyTag);
    }
    ...
    inline ETrackedPropertyError
    CVRPropertyHelpers::SetInt32Property(PropertyContainerHandle_t ulContainerHandle,
                                         ETrackedDeviceProperty prop,
                                         int32_t nNewValue) {
        return SetProperty(
    ...
    inline ETrackedPropertyError
    CVRPropertyHelpers::SetUint64Property(PropertyContainerHandle_t ulContainerHandle,
                                          ETrackedDeviceProperty prop,
                                          uint64_t ulNewValue) {
        return SetProperty(
...
namespace vr {
    ...
    class IVRServerDriverHost {
      public:
        ...
        /** Notifies the server that vsync has occurred on the the display attached to the device.
         * This is only permitted on devices of the HMD class. */
        ...
        /** Returns true and fills the event with the next event on the queue if there is one. If
         * there are no events this method returns false. uncbVREvent should be the size in bytes of
         * the VREvent_t struct */
        virtual bool PollNextEvent(VREvent_t *pEvent, uint32_t uncbVREvent) = 0;

        /** Provides access to device poses for drivers.  Poses are in their "raw" tracking space
         * which is uniquely defined by each driver providing poses for its devices.  It is up to
         * clients of this function to correlate poses across different drivers.  Poses are indexed
         * by their device id, and their associated driver and other properties can be looked up via
         * IVRProperties. */
        ...
        /** Notifies the server that a tracked device's display component transforms have been
         * updated. */
...
namespace vr {
    ...
    class IVRResources {
      public:
         ...
         * actual physical path. pchResourceTypeDirectory is the subdirectory of resources to look
         * in. */
...
namespace vr {
    ...
    class COpenVRDriverContext {
      public:
        COpenVRDriverContext() : m_propertyHelpers(nullptr), m_hiddenAreaHelpers(nullptr) {
            Clear();
        }
        void Clear();
