...
class HMD : public ITrackedDeviceServerDriver,
  ...
  public:
    HMD(string ip)
        : talker(
              PVRProp<uint16_t>({CONN_PORT_KEY}),
              [=](auto msgType, auto data) {
                  ...
                  if (msgType == PVR_MSG::DISCONNECT) {
                      ...
                      // auto oldTm = Clk::now();
                      // while (objId == k_unTrackedDeviceIndexInvalid && Clk::now() - oldTime < 3s)
                      // // 7s - ensure propCont is set 	sleep_for(10ms);

                      // PVR_DB_I("[HMD::talker]: ADD DATA MSG RCV'ed.. propCont seems to be set or
                      // timeout"); if (objId == k_unTrackedDeviceIndexInvalid)
                      {
                          auto ui16Data = reinterpret_cast<uint16_t *>(&data[0]);
                          rdrW = ui16Data[0];
                          rdrH = ui16Data[1];
                          memcpy(projRect, &data[2 * 2], 4 * 4);
                          PVR_DB_I("[HMD::talker]: addData: Viewport:  left: " +
                                   to_string(projRect[0]) + "  top: " + to_string(projRect[1]) +
                                   "  right: " + to_string(projRect[2]) +
                                   "  bottom: " + to_string(projRect[3]));
                          ipd = *reinterpret_cast<float *>(&data[2 * 2 + 4 * 4]);

                          // VRProperties()->SetFloatProperty(propCont, Prop_UserIpdMeters_Float,
                          // ipd);
    ...
    virtual DriverPose_t GetPose() override { return pose; }
    ...
    virtual EVRInitError Activate(uint32_t objectId) override {
        ...
        VRProperties()->SetBoolProperty(propCont,
                                        Prop_DeviceProvidesBatteryStatus_Bool,
                                        false);   // TODO implement and then set to true
        VRProperties()->SetBoolProperty(propCont, Prop_DeviceCanPowerOff_Bool, true);
        // VRProperties()->SetInt32Property(propCont, Prop_DeviceClass_Int32,
        // TrackedDeviceClass_HMD);
        VRProperties()->SetBoolProperty(propCont, Prop_HasCamera_Bool, false);
        VRProperties()->SetStringProperty(
            propCont, Prop_DriverVersion_String, versunint2str(PVR_SERVER_VERSION).c_str());
        VRProperties()->SetBoolProperty(
            propCont, Prop_Firmware_ForceUpdateRequired_Bool, false);   // TODO implement
        ...
        VRProperties()->SetFloatProperty(
            propCont, Prop_DisplayFrequency_Float, PVRProp<float>({GAME_FPS_KEY}));
        VRProperties()->SetUint64Property(
            propCont, Prop_CurrentUniverseId_Uint64, /*P+V+R ascii = */ 0x808682);
        ...
        // VRProperties()->SetBoolProperty(propCont, Prop_DisplaySuppressed_Bool, false); // ??
        // false
        VRProperties()->SetBoolProperty(propCont, Prop_DisplayAllowNightMode_Bool, true);   // ??

        VRProperties()->SetStringProperty(
            propCont, Prop_NamedIconPathDeviceOff_String, "{PVRServer}/icons/hmd_off.png");
        VRProperties()->SetStringProperty(
            propCont, Prop_NamedIconPathDeviceSearching_String, "{PVRServer}/icons/hmd_alert.png");
        VRProperties()->SetStringProperty(propCont,
                                          Prop_NamedIconPathDeviceSearchingAlert_String,
                                          "{PVRServer}/icons/hmd_alert.png");
        VRProperties()->SetStringProperty(
            propCont, Prop_NamedIconPathDeviceReady_String, "{PVRServer}/icons/hmd_ready.png");
        VRProperties()->SetStringProperty(propCont,
                                          Prop_NamedIconPathDeviceReadyAlert_String,
                                          "{PVRServer}/icons/hmd_ready_alert.png");
        VRProperties()->SetStringProperty(
            propCont, Prop_NamedIconPathDeviceNotReady_String, "{PVRServer}/icons/hmd_error.png");
        VRProperties()->SetStringProperty(
            propCont, Prop_NamedIconPathDeviceStandby_String, "{PVRServer}/icons/hmd_standby.png");
        VRProperties()->SetStringProperty(propCont,
                                          Prop_NamedIconPathDeviceAlertLow_String,
                                          "{PVRServer}/icons/hmd_ready_low.png");
        ...
        if (addDataRcvd) {
            PVR_DB_I(
                "[Activating HMD]: Rcvd addotionalData... Starting PVRStartStreamer with [WxH]:" +
                to_string(rdrW) + "x" + to_string(rdrH));
            ...
            PVRStartStreamer(
                ...
                rdrW,
                rdrH,
                [=](auto v) { talker.send(PVR_MSG::HEADER_NALS, v); },
                [=] { terminate(); });
            ...
            PVR_DB_I("[Activating HMD]: Timeout didnt recv additionalData... for " +
                     to_string(timeout.count()) + "secs");
    ...
    virtual void EnterStandby() override { PVR_DB_I("[Open VR EnterStandby] HMD enter standby"); }
    ...
    GetWindowBounds(int32_t *x, int32_t *y, uint32_t *width, uint32_t *height) override {
        ...
        PVR_DB_I("[Open VR GetWindowBounds] Returning w " + to_string(*width) + " h " +
                 to_string(*height));
        *x = 0;
        *y = 0;
        *width = rdrW;
    ...
    virtual void GetRecommendedRenderTargetSize(uint32_t *width, uint32_t *height) override {
        ...
        PVR_DB_I("[Open VR GetRecommendedRenderTargetSize] Returning w " + to_string(*width) +
                 " h " + to_string(*height));
        *width = rdrW;
        *height = rdrH;
    }
    ...
    virtual void GetEyeOutputViewport(
        EVREye eye, uint32_t *x, uint32_t *y, uint32_t *width, uint32_t *height) override {
        *width = rdrW / 2;
        *height = rdrH;
        *x = rdrW / 2 * eye;
        *y = 0;
    ...
    virtual void
    GetProjectionRaw(EVREye eye, float *left, float *right, float *top, float *bottom) override {
    ...
    virtual void Present(SharedTextureHandle_t backBuffer) override {
        ...
        // newFrameQuat = Quaternionf(latestQuat); // poll here the quaternion used in the next
        // frame VRServerDriverHost()->TrackedDevicePoseUpdated(objId, GetPose(),
        // sizeof(DriverPose_t)); waitForPresent = false;
...
class ServerProvider : public IServerTrackedDeviceProvider {
  ...
  public:
    virtual EVRInitError Init(IVRDriverContext *drvCtx) override {
        ...
        PVRStartConnectionListener([&](auto ip, auto msgType) {
            if (msgType == PVR_MSG::PAIR_HMD && !hmd) {
                ...
                PVR_DB_I("[ServerProvider::Init::PVRStartConnectionListener] recvd PAIR_HMD Msg "
                         "from ip: " +
                         to_string(ip));
                if (eError == vr::VRInitError_None) {
                    ...
                    // red = new Redirect();
                    hmdPaired =
                        VRServerDriverHost()->TrackedDeviceAdded("0", TrackedDeviceClass_HMD, hmd);
                    // hmdPaired = VRServerDriverHost()->TrackedDeviceAdded("1",
                    // TrackedDeviceClass_DisplayRedirect, red);
                    if (!hmdPaired) {
                        PVR_DB_I("[ServerProvider::Init::PVRStartConnectionListener] Error: could "
                                 "not activate HMD");
                    } else
                        hmdBomb.defuse();
                } else {
                    PVR_DB_I("[ServerProvider::Init::PVRStartConnectionListener] Error: could not "
                             "InitServerDriverContext");
                }
            } else if (msgType == PVR_MSG::PAIR_PHONE_CTRL) {
                if (find(IPs.begin(), IPs.end(), ip) == IPs.end()) {
                    IPs.insert(ip);
                    PVR_DB_I("[ServerProvider::Init::PVRStartConnectionListener] Recvd "
                             "PAIR_PHONE_CTRL from ip: " +
                             to_string(ip));
                    // init controller
                    // VRServerDriverHost()->TrackedDeviceAdded(to_string(ctrls.size()).c_str(),
                    // TrackedDeviceClass_Controller, ctrl); ctrls.insert(ctrl)
                }
            }
        });
        ...
        if (!hmdPaired) {
            ...
            PVR_DB_I("[ServerProvider::Init] Pairing failed! Cause: timeout ... for " +
                     to_string(timeout.count()) + "secs");
    ...
    virtual void RunFrame() override {
        ...
        //             VRServerDriverHost()->TrackedDevicePoseUpdated(hmd->objId, hmd->GetPose(),
        //             sizeof(DriverPose_t));
        //         }
        // #endif
    }
    ...
    virtual void EnterStandby() override { PVR_DB_I("Server enter standby"); }

    virtual void LeaveStandby() override { PVR_DB_I("Server leave standby"); }
};

static ServerProvider server;

extern "C" __declspec(dllexport) void *HmdDriverFactory(const char *iName, int *retCode) {
    /*if (PVRCheckIfExpired()) {
            PVR_DB("This version of PhoneVR driver has expired. Please install a more recent
    version"); *retCode = VRInitError_Driver_Failed; return nullptr;
    }*/
