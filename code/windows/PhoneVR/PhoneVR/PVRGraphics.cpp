...
#define OK_OR_EXEC(call, func)                                                                     \
    if (FAILED(call))                                                                              \
    func()
#define OK_OR_DEBUG(call)                                                                          \
    {                                                                                              \
        HRESULT hr = call;                                                                         \
        OK_OR_EXEC(hr, [hr] {                                                                      \
...
#define QUERY(from, to)                                                                            \
    OK_OR_DEBUG(from->QueryInterface(__uuidof(to), reinterpret_cast<void **>(&to)))
// #define PARENT(child, parent) OK_OR_DEBUG(child->GetParent(__uuidof(parent),
// reinterpret_cast<void**>(&parent))) #define OK_OR_REINIT(call) OK_OR_EXEC(call, [] {
// PVRReleaseDX(); PVRInitDX(outID); })
...
void PVRInitDX() {
    ...
    OK_OR_DEBUG(D3D11CreateDevice(nullptr,
...
void PVRUpdTexHdl(uint64_t texHdl, int whichBuf) {
    if (gRunning) {
        // wait to obtain TextureMutex lock = Wait for the Renderer to complete rendering of present
        // frame
        texMtx.lock();
        ::whichBuf = whichBuf;
        curHdl = texHdl;
...
void PVRStartGraphics(vector<vector<uint8_t *>> vvbuf, uint32_t inpWidth, uint32_t inpHeight) {
    ...
    gThr = new thread([=] {
        ...
        for (auto vbuf : vvbuf)   // divide width by 4: store 4 bytes in an uint
            yuvBufViews.push_back(
                {array_view<uint, 2>(inpHeight, inpWidth / 4, reinterpret_cast<uint *>(vbuf[0])),
                 array_view<uint, 2>(
                     inpHeight / 2, inpWidth / 4 / 2, reinterpret_cast<uint *>(vbuf[1])),
                 array_view<uint, 2>(
                     inpHeight / 2, inpWidth / 4 / 2, reinterpret_cast<uint *>(vbuf[2]))});
        ...
        while (gRunning) {
            ...
            OK_OR_DEBUG(dxDev->OpenSharedResource(
                (HANDLE) curHdl, __uuidof(ID3D11Texture2D), (void **) &inpTex));
            ...
            concurrency::parallel_for_each(
                concurrency::extent<2>(inpHeight / 2, inpWidth / 8),
                [ =, &ampTex ](index<2> idx) restrict(amp) {
                    ...
                    for (int y = 0; y < 2; y++)
                        for (int x = 0; x < 2; x++)
                            outY[idx * 2 + index<2>(y, x)] =
                                yuv[y][x * 4][0] | yuv[y][x * 4 + 1][0] << 8 |
                                yuv[y][x * 4 + 2][0] << 16 | yuv[y][x * 4 + 3][0] << 24;
				...
				// \ | /\ | /\ | /\ | /
                    ...
                    outU[idx] =
                        yuv[0][0][1] | yuv[0][2][1] << 8 | yuv[0][4][1] << 16 | yuv[0][6][1] << 24;
                    outV[idx] =
                        yuv[0][0][2] | yuv[0][2][2] << 8 | yuv[0][4][2] << 16 | yuv[0][6][2] << 24;
                });
            outY.synchronize();
            outU.synchronize();
            outV.synchronize();
