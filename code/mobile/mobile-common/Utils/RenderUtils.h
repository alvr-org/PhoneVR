...
class Renderer {
    ...
    void
    initProg(std::vector<std::pair<GLuint, bool>> inTexs, std::string vs, std::string fragBody);

  public:
    // inTexs: {tex uniform, is oes}
    // RTS
    Renderer(std::vector<std::pair<GLuint, bool>> inTexs,
             std::string fragBody = FS_PT,
             float xStart = .0,
             float xEnd = 1.0);
    ...
    Renderer(int outWidth,
             int outHeight,
             std::vector<std::pair<GLuint, bool>> inTexs,
             std::string fragBody,
             GLenum fmt,
             GLint mag,
             GLuint rdrBuf = 0,
