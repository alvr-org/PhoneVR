...
public class DiscordReportSender implements ReportSender {
    ...
    @Override
    public void send(@NotNull Context context, @NotNull CrashReportData report, @NotNull BundleWrapper extras)
    {
        try {
            ...
            try {
                ...
                while(m.find()) {
                    pvrRelatedFiles.append(" --> " +
                            m.group(0)
                                    .replaceAll("\n|\t", "")
                                    .replaceAll("\\((.*?):([0-9]*?)\\).*?", "([$1:$2](" + gitFileUrl + "$1#L$2))") +
                            "\n");
                }
                JSONObject JsonPayload = new JSONObject();

                JsonPayload.put("embeds", new JSONArray()
                        .put(new JSONObject()
                                .put("author", new JSONObject()
                                        .put("name", "Github CrossRef")
                                        .put("url", "https://github.com/ShootingKing-AM")
                                        .put("icon_url","https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png"))
                                .put("footer", new JSONObject()
                                        .put("icon_url", "https://avatars0.githubusercontent.com/u/4137788?s=100" )
                ...
                JsonPayload.put("content", "An `" +
                        ...
                        "(" + ((JSONObject)reportsData.get("INITIAL_CONFIGURATION")).get("locale") + ") `" +
                        (new String(Character.toChars(firstLetter)) + new String(Character.toChars(secondLetter))) +
                        "  crashed on ```css\n" +
                        "Time:           " + reportsData.get("USER_CRASH_DATE") + "\n" +
                        "App Version:    " + ((JSONObject)reportsData.get("BUILD_CONFIG")).get("VERSION_NAME") + "_" + ((JSONObject)reportsData.get("BUILD_CONFIG")).get("VERSION_CODE") +
                        "```*With:*\n```ml\n" + ExceptionMsg +
                        "```\n");

                writer.write("--" + boundary + "\r\n");
                writer.write("Content-Disposition: form-data; name=\"file\"; filename=\"pvrcrash.stacktrace\" \r\n\r\n");
                ...
                Log.d("ACRA-PVR", "Connection Resp Code : " + urlConnection.getResponseCode()  + "; Resp Msg : " + urlConnection.getResponseMessage()) ;
