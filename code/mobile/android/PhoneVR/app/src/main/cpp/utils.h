...
void log(AlvrLogLevel level, const char *FILE_NAME, unsigned int LINE_NO, const char *FUNC, const char *format, ...) {
    va_list args;
    va_start(args, format);

    char buf[1024];
    std::string format_str;
    format_str = std::string(FILE_NAME) + ":"  + std::to_string(LINE_NO) + ": "+ std::string (FUNC) + "():" + format;
